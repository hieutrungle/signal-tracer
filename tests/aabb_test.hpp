...
TEST(AABBTest, AABBUninitilization) {
    SignalTracer::AABB aabb{};
    // EXPECT_EQ(aabb.length(), (glm::vec3{0.0f, 0.0f, 0.0f}));
    EXPECT_FALSE(aabb.is_contained(glm::vec3{ 0.0f, 0.0f, 0.0f }));
...
TEST(AABBTest, AABBConstructorOrdered) {
    ...
    SignalTracer::AABB aabb{ p1, p2 };
    // EXPECT_EQ(aabb.length(), (glm::vec3{ 3.0f, 3.0f, 3.0f }));
    EXPECT_TRUE(aabb.is_contained(p1));
    EXPECT_TRUE(aabb.is_contained(p2));
    EXPECT_TRUE(aabb.is_contained(glm::vec3{ 2.5f, 3.5f, 4.5f }));
    // EXPECT_TRUE(aabb.surrounds(glm::vec3{ 2.5f, 3.5f, 4.5f }));
    EXPECT_TRUE(aabb == (SignalTracer::AABB{ p1, p2 }));
}
...
TEST(AABBTest, AABBContaining) {
    ...
    EXPECT_TRUE(aabb.is_contained(p2));
    EXPECT_TRUE(aabb.is_contained(glm::vec3{ 2.5f, 3.5f, 4.5f }));
    EXPECT_FALSE(aabb.is_contained(glm::vec3{ 0.5f, 0.5f, 0.5f }));
    EXPECT_FALSE(aabb.is_contained(glm::vec3{ 7.5f, 7.5f, 7.5f }));
}
...
TEST(AABBTest, AABBCombinationTwoAABBs) {
    ...
    SignalTracer::AABB aabb{ p1, p2 };
    SignalTracer::AABB aabb2{ glm::vec3{2.0f, 3.0f, 4.0f}, glm::vec3{5.0f, 6.0f, 7.0f} };
    SignalTracer::AABB aabb3{ aabb, aabb2 };
    EXPECT_TRUE(aabb3 == (SignalTracer::AABB{ glm::vec3{1.0f, 2.0f, 3.0f}, glm::vec3{5.0f, 6.0f, 7.0f} }));
}
...
TEST(AABBTest, AABBCombinationThreeAABBs) {
    ...
    SignalTracer::AABB aabb{ p1, p2, p3 };
    glm::vec3 p4{ 2.0f, 3.0f, 4.0f };
    glm::vec3 p5{ 5.0f, 6.0f, 7.0f };
    glm::vec3 p6{ 4.0f, 7.0f, 5.0f };
    SignalTracer::AABB aabb2{ p4, p5, p6 };
    glm::vec3 p7{ 3.0f, 9.0f, 5.0f };
    glm::vec3 p8{ 6.0f, 7.0f, 8.0f };
    glm::vec3 p9{ 5.0f, 8.0f, 6.0f };
    SignalTracer::AABB aabb3{ p7, p8, p9 };
    SignalTracer::AABB aabb4{ aabb, aabb2, aabb3 };
    EXPECT_TRUE(aabb4 == (SignalTracer::AABB{ glm::vec3{1.0f, 2.0f, 3.0f}, glm::vec3{6.0f, 9.0f, 8.0f} }));
}
...
TEST(AABBTest, AABBCombinationBoxPoint) {
    ...
    SignalTracer::AABB aabb2{ aabb, p4 };
    EXPECT_TRUE(aabb2 == (SignalTracer::AABB{ glm::vec3{1.0f, -1.0f, 3.0f}, glm::vec3{4.0f, 6.0f, 9.0f} }));
}
